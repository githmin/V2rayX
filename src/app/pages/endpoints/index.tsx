...
import { useState } from 'react';
import { Button, Modal, useModal, Tooltip, Text } from '@nextui-org/react';
import { uniqBy, findIndex, cloneDeep } from 'lodash';
import { useAppSelector, useAppDispatch } from '@store/hooks';
import { setServersGroups } from '@store/serversPageSlice';
import { Subscription } from '@renderer/constant/types';
...
const actions = [
  { icon: '📝', name: 'Create From Boilerplate' },
  { icon: '📷', name: 'Scan QR Code From Screen (Upcoming)' },
  { icon: '📋', name: 'Import From Clipboard' },
  { icon: '🔗', name: 'Manage Subscriptions' },
];
...
const Index = () => {
  ...
  const handleImportFrom = (type) => {
    if (type === 'clipboard') {
      window.clipboard.read().then((link) => {
        const factory = link.includes('vmess://')
          ? new VMess(link)
          : link.includes('vless://')
            ? new VLess(link)
  ...
  return (
    <Container fluid display="flex" direction="column" alignItems="center" justify="center">
      <Grid.Container gap={2} justify="center">
        <FoldList
          handleCheckDeleteDialog={(v, groupId) => {
            setOpen(v);
            deleteItemGroupId = groupId;
          }}
        />
        {actions.map((action) => (
          <Tooltip content={action.name} key={action.name} placement="right">
            <Button
              auto
              shadow
              onClick={() => {
      ...
      <Modal {...bindings}>
        <Modal.Title>Delete subscription item?</Modal.Title>
        <Modal.Body>
          <Text>
            Are you sure delete this subscription? It would also delete subscription item in
            subscription page.
